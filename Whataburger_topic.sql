...
, topic.name as "Topic"
, row_number() over(partition by c.name, ag.name order by sum(d.video_views)/sum(d.impressions)) as "Topic Rank"
...
left join(select ad_group_id, topic_id,  date, impressions, clicks, video_views, cost, video_played_to_25_percent, video_played_to_50_percent, video_played_to_75_percent, video_played_to_100_percent, conversions, all_conversions
 from adwords.aw_topic_summary_daily where impressions > 0 and date >= '2019-01-01') as d 
on d.ad_group_id=ag.ad_group_id

left join (select topic_id, name from adwords.aw_topic) as topic 
on d.topic_id=topic.topic_id
