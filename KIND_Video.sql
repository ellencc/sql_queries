...
, ad.video_ad_format as "Video Ad Format"
, d.date as "Date"
, sum(d.impressions) as "Impressions"
, sum(d.clicks) as "Clicks"
, sum(d.video_views) as "Views"
, sum(case when ad.video_ad_format = 'Discovery' then (d.video_played_to_25_percent/100)*d.video_views
           else (d.video_played_to_25_percent/100)*d.impressions 
      end) as "Played to 25%"
, sum(case when ad.video_ad_format = 'Discovery' then (d.video_played_to_50_percent/100)*d.video_views
           else (d.video_played_to_50_percent/100)*d.impressions 
      end) as "Played to 50%"
, sum(case when ad.video_ad_format = 'Discovery' then (d.video_played_to_75_percent/100)*d.video_views
           else (d.video_played_to_75_percent/100)*d.impressions 
      end) as "Played to 75%"
, sum(case when ad.video_ad_format = 'Discovery' then (d.video_played_to_100_percent/100)*d.video_views
           else (d.video_played_to_100_percent/100)*d.impressions 
      end) as "Played to 100%"
, sum(case when ad.video_ad_format = 'Discovery' then d.video_views
       else d.impressions
...
left join (select ad_group_id, ad_group_ad_id, name, youtube_video_title, youtube_video_duration as ad_length, case when youtube_video_duration/1000 <=7 then 'Bumper' when video_ad_format= 'INDISPLAY' then 'Discovery' else 'InStream' end as video_ad_format from adwords.aw_ad_group_ad) as ad
on ad.ad_group_id=ag.ad_group_id

left join (select ad_group_ad_id, date, impressions, clicks, video_views, cost, video_played_to_25_percent, video_played_to_50_percent, video_played_to_75_percent, video_played_to_100_percent from adwords.aw_ad_group_ad_summary_daily where impressions >0 and date>= '2018-01-01') as d
on d.ad_group_ad_id=ad.ad_group_ad_id
