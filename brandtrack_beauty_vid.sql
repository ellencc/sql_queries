...
, stats.video_title as "Video Title"
, stats.yt_video_id as "Video ID"
, dateadd('hour',5,date_trunc('month',stats.date)) as "Month"
, dateadd('hour',5,date_trunc('quarter',stats.date)) as "Quarter"
, max(as_integer(stats.views)) as "Total Views"
, max(pvp.paid_view_percentage) * max(as_integer(stats.views)) as paid_views

from (select channel_title, yt_channel_id, video_title, yt_video_id, date, views from  thresher.yt_video_stats_over_time where date >= '2018-01-01') as stats

left join (select yt_video_id, paid_view_percentage from thresher.yt_video_stats) as pvp
on pvp.yt_video_id=stats.yt_video_id

where stats.date >= '2018-01-01'
and stats.yt_channel_id in (
