...
, case when sum(sum(d.video_views*io.rate)) over (partition by io.campaign_name order by d.date, c.name, ag.name, gender.name)  < io.cost then sum(d.video_views*io.rate)
       else greatest(0,sum(d.video_views*io.rate)-((sum(sum(d.video_views*io.rate)) over (partition by io.campaign_name order by d.date, c.name, ag.name, gender.name))-io.cost))
  end as "Client Spend"
...
left join (select ad_group_id, gender_id, to_date(date) as date, impressions, video_views, clicks, cost as spend, video_played_to_25_percent/100 as played_to_25, video_played_to_50_percent/100 as played_to_50 , video_played_to_75_percent/100 as played_to_75 , video_played_to_100_percent/100 as played_to_100  from adwords.aw_gender_summary_daily where impressions >0) as d
on ag.ad_group_id=d.ad_group_id

left join (select gender_id, name from adwords.aw_gender) as gender
on gender.gender_id=d.gender_id
