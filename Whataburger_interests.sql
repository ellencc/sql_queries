...
, row_number() over(partition by c.name, ag.name order by sum(d.video_views)/sum(d.impressions)) as "Interest Rank"
...
left join (select campaign_name, opportunity_id, flight_start_date, flight_end_date, platform, cost_structure,units, cost, rate, media_budget, fee from salesforce.iolineitem where deleted = 'false') as io
...
left join(select ad_group_id, interest_id,  date, impressions, clicks, video_views, cost, video_played_to_25_percent, video_played_to_50_percent, video_played_to_75_percent, video_played_to_100_percent, conversions, all_conversions
from adwords.aw_audience_summary_daily where impressions > 0 and date >= '2019-01-01') as d 
on d.ad_group_id=ag.ad_group_id

left join (select interest_id, name from adwords.aw_interest) as interest 
on d.interest_id=interest.interest_id
